...
const DoctorOnlineAppointments = () => {
  ...
  const handleJoinCall = (meetingLink, callType) => {
    if (meetingLink) {
      window.open(meetingLink, '_blank');
    } else {
      alert(`${callType} meeting link not available yet`);
    }
  };
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const response = await fetch(
        `https://physiocareapp.runasp.net/api/v1/PatientBookingDoctorOnlines/get-all-booking-online-by-patient-id/${patientId}`,
        {
          headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch online doctor appointments');
      }
      ...
      const transformedAppointments = data.map(appointment => ({
        ...
        doctorName: appointment.doctorName || 'Dr. Unknown',
        specializationNameAR: appointment.nameAR || appointment.doctorSpecialtyAR || '',
        specializationNameEN: appointment.nameEN || appointment.doctorSpecialty || 'General Practice',
        date: appointment.date || appointment.appointmentDate,
        time: appointment.time || appointment.appointmentTime,
        medicalCondition: appointment.medicalCondition || appointment.notes || appointment.reason || '',
        totalPrice: appointment.totalPrice || appointment.cost || appointment.fee || 0,
        isAccepted: appointment.isAccepted || false,
        consultationType: appointment.consultationType || appointment.type || 'Video Call',
        meetingLink: appointment.meetingLink || appointment.videoLink || ''
      }));

      setAppointments(transformedAppointments);
      setError(null);
    } catch (err) {
      setError('Failed to fetch online doctor appointments');
      console.error('Error fetching online doctor appointments:', err);
      setAppointments([
        {
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      const response = await fetch(
        `https://physiocareapp.runasp.net/api/v1/PatientBookingDoctorOnlines/soft-delete-booking-by-id/${selectedBookingId}?cancelReason=${encodeURIComponent(cancelReason)}`,
        {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${localStorage.getItem('accessToken')}`
      ...
      <Container maxWidth="lg" sx={{ py: 4, minHeight: '80vh' }}>
        <Box sx={{ mb: 4 }}>
          ...
          <Typography variant="body1" color="text.secondary">
            Manage your online consultations and telemedicine appointments
          </Typography>
        </Box>
        ...
        <StyledTableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label="online doctor appointments table">
            <StyledTableHead>
              <TableRow>
        ...
        {appointments.length === 0 && (
          <Box sx={{ textAlign: 'center', py: 4 }}>
            ...
            <Typography variant="body2" color="text.secondary">
              Your online doctor consultations will appear here
            </Typography>
          </Box>
        )}

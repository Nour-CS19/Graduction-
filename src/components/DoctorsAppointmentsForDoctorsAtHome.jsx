...
const DoctorHomeAppointments = () => {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const response = await fetch(
        `https://physiocareapp.runasp.net/api/v1/PatientBookingDoctorAtHomes/get-all-booking-at-home-by-patient-id/${patientId}`,
        {
          headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch home doctor appointments');
      }
      ...
      setError('Failed to fetch home doctor appointments');
      console.error('Error fetching home doctor appointments:', err);
      setAppointments([
        {
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      const response = await fetch(
        `https://physiocareapp.runasp.net/api/v1/PatientBookingDoctorAtHomes/soft-delete-booking-by-id/${selectedBookingId}?cancelReason=${encodeURIComponent(cancelReason)}`,
        {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${localStorage.getItem('accessToken')}`
      ...
      <Container maxWidth="lg" sx={{ py: 4, minHeight: '80vh' }}>
        <Box sx={{ mb: 4 }}>
          ...
          <Typography variant="body1" color="text.secondary">
            Manage your doctor appointments at home
          </Typography>
        </Box>
        ...
        <StyledTableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label="home doctor appointments table">
            <StyledTableHead>
              <TableRow>
        ...
        {appointments.length === 0 && (
          <Box sx={{ textAlign: 'center', py: 4 }}>
            <Typography variant="h6" color="text.secondary">
              No home appointments found
            </Typography>
            <Typography variant="body2" color="text.secondary">
              Your home doctor appointments will appear here
            </Typography>
          </Box>
        )}
...
export default DoctorHomeAppointments;
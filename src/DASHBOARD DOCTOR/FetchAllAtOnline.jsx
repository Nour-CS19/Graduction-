...
const API_SERVICE = {
  ...
  async fetchReceiptImage(filePath) {
    ...
    try {
      const response = await fetch(`${this.BASE_URL}/Upload/image?filename=${encodeURIComponent(filePath)}&path=Doctor%2FBooking%2FOnline`, {
        method: 'GET',
...
const BookingManagementPage = () => {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      const endpoint = `/PatientBookingDoctorOnlines/get-all-booking-online/${user.id}`;
  ...
  const handleBookingAction = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_SERVICE.BASE_URL}/PatientBookingDoctorOnlines/${actionBooking.bookingId}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleSoftDelete = async () => {
    ...
    try {
      ...
      let endpoint = `/PatientBookingDoctorOnlines/soft-delete-booking-by-id/${deleteBookingId}`;
  ...
  const handleHardDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_SERVICE.BASE_URL}/PatientBookingDoctorOnlines/hard-delete-booking-by-id/${deleteBookingId}`, {
        method: 'DELETE',
        headers: API_SERVICE.getAuthHeaders()
      });
  ...
  return (
    <div className="min-vh-100 bg-light py-4">
      <div className="container">
        ...
        {alert.message && (
          <div className={`alert alert-${alert.type === 'success' ? 'success' : alert.type === 'danger' ? 'danger' : alert.type === 'info' ? 'info' : 'warning'} alert-dismissible fade show mb-4`}>
            {alert.message}
              ...
              <table className="table table-hover align-middle">
                <thead className="table-light">
                  <tr>
                    ...
                    <th>
                      <Clock className="me-2" size={16} /> Time
                    </th>
                    <th>
